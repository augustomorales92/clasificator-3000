...
export default function PrescriptionDetailsPage({ id }: PageProps) {
  ...
  const { task, isLoading } = useGeminiTasks(id)

  // Show loading state instead of 404 while data is being fetched
  if (isLoading) {
    return (
  ...
  // Only show 404 after we're sure the data is not loading and doesn't exist
  if (!task) {
    notFound()
  }
